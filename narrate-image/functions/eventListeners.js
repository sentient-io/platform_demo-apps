...
handleImageProcessing = () => {
	...
	objectDetectionAPICall(data)
		...
		.then((result) => {
			...
			$('#toggleBoxes').show()
			$('#btn-generateAudio').show()
			state.detectionDescription = result
			loadingEnd()
		})
		.catch((err) => {
			// Toggle popup window
...
/* +-------------------------+ */
handleAudio = () => {
	$('#btn-main-function, #generateAudio').hide()
	$('#in-line-loader').show()
	textToSpeechAPICall(state.detectionDescription)
		.then((result) => {
			var audioString = 'data:audio/wav;base64,' + result.results['wav_base64']
		...
		.finally(() => {
			$('#in-line-loader, #btn-generateAudio').hide()
		})
}
...
handleRestart = () => {
	...
	$(
		'#btn-restart, #toggleBoxes, #btn-upload-another, #btn-main-function, #btn-generateAudio, #audio, #detectionDescription'
	).hide()
	$('#s-img-input').val('')
